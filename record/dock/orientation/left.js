...
module.exports = {
  run: async (outputPath) => {
    ...
    const { stderr: setEnvError } = await exec('defaults write com.apple.screencapture show-thumbnail -bool false && defaults write com.apple.dock orientation -string left && killall Dock')
    if (setEnvError) {
    ...
    try {
      await compressPngImage(screenshot, outputPath, 'left')
