...
module.exports = {
  run: async (outputPath) => {
    ...
    const { stderr: setEnvError } = await exec(`defaults write com.apple.screencapture show-thumbnail -bool false && defaults write com.apple.dock persistent-apps -array-add '{"tile-type"="spacer-tile";}' && killall Dock`)
    if (setEnvError) {
    ...
    try {
      await compressPngImage(screenshot, outputPath, 'spacer-tile')
    ...
    return { filepath: `${outputPath}/spacer-tile` }
  }
}
