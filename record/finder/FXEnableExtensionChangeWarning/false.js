...
// Using Magnet, put the Finder in Top Left then Center and drag it up until getting 345 pixels from bottom (measure with cmd+shift+5)
// Make sure "Snap windows by dragging" setting is off
module.exports = {
  run: async (outputPath) => {
    ...
    robot.typeString('~/macos-defaults-list/')
    await delay(1000)
    robot.keyTap('enter')
    await delay(500)
    robot.keyTap('right')
    ...
    robot.keyTap('left', ['shift', 'alt'])
    await delay(100)
    robot.typeStringDelayed('yml', 300)
    await delay(100)
    robot.keyTap('enter')
    await delay(300)
