...
module.exports = {
  run: async (outputPath) => {
    ...
    const { stderr: setEnvError } = await exec(`defaults write com.apple.systemuiserver menuExtras "('/System/Library/CoreServices/Menu Extras/Clock.menu')" && defaults write com.apple.menuextra.clock FlashDateSeparators -bool true && killall SystemUIServer`)
    ...
    // End recording
    try {
      await compressVideo(fp, outputPath, 'FlashDateSeparators')
    ...
    return { filepath: `${outputPath}/FlashDateSeparators`, isVideo: true }
  }
}
