  ...
  if (defaults.categories !== null) {
    ...
    defaults.categories.forEach(({ folder, name, keys }) => {
      keys.forEach(({ domain, ...page }) => {
        mkdirSyncIfNeeded(`${destinationPath}/docs/${folder}`)
        ...
        page.examples.forEach(example => {
          if (example.image !== undefined) {
            mkdirSyncIfNeeded(`${destinationPath}/docs/assets/${folder}`)
            mkdirSyncIfNeeded(`${destinationPath}/docs/assets/${folder}/${page.key}`)
            fs.copyFileSync(
              `../../images/${folder}/${page.key}/${example.image}`,
              `${destinationPath}/docs/assets/${folder}/${page.key}/${example.image}`
            )
          }
        })
      })
